class DD_AugmentationUpgradeCanisterLegendary : DD_AugmentationUpgradeCanister
{
	...
	// Description:
	//	Tells whether a legendary augmentation canister can be consumed or not (for UI)
	static clearscope bool canConsume(PlayerPawn plr, DD_AugmentationUpgradeCanisterLegendary cnst_instance, int aug_slot)
	{
		DD_AugsHolder aughld = DD_AugsHolder(plr.findInventory("DD_AugsHolder"));
		...
		return cnst_instance
		    ...
		    && plr.countInv("DD_AugmentationUpgradeCanisterLegendary") >= cnst_instance.queue.toupgrade.size()
		    && plr.countInv("DD_AugmentationUpgradeCanisterLegendary") > 0;
	}
}
