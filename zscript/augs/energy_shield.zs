class DD_Aug_EnergyShield : DD_Augmentation
{
	...
	DD_EnergyShield_AuraEffect aura_effect;
	...
	const dmg_aura_threshold = 20; // minimum damage to actually harm things around
	const dmg_aura_range = 64; // added to the owner's radius
	...
	override void Tick()
	{
		...
		if(legendary && dmg_resv > dmg_aura_threshold){
			...
			vector3 ae_warp_off = owner.vel;
			aura_effect.warp(owner, -1, 0, 0, 0, WARPF_NOCHECKPOSITION);
		}
		else if(aura_effect){
			aura_effect.Destroy();
			aura_effect = null;
		...
		if(dmg_resv > dmg_aura_threshold){
			...
			while(itb.next())
			{
				...
				dealt_dmg = true;
				mnst.damageMobj(owner, owner, 1, "Fire",
...
class DD_EnergyShield_AuraEffect : Actor
{
	default
	{
		+NOBLOCKMAP
	...
	const dmg_aura_maxopaq_resv = 200.0; // Damage reserve that makes effect fully opaque
	...
	override void Tick()
	{
		...
		A_SetRenderStyle(1 - opaq, STYLE_Translucent);
